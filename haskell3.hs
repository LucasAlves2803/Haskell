...
avaliaExp (mem,cod) (App id e)  = ret where
                                    ...
                                    fun  = traceShow(cod) consulta cod id -- fun recebe o código da função
avaliaExp (mem,cod) (App2 id (e1,e2)) = ret where
                                     ret = consulta memx "ret"
									 (memx ,_) = avaliaCmd (mem'',cod) fun
									 mem'' = escreve mem' "arg2" arg2 
									 mem' = escreve mem "arg" arg1
									 arg1 = avaliaExp (mem,cod) (e1,_)
									 arg2 = avaliaExp (mem,cod) (_,e2)
									 fun = consulta cod id 
...
avaliaCmd (mem,cod) (Fun id x c)    = ( mem, escreve cod id cmd') where
                                         cmd' = Seq (Atr x (Var "arg"))
													(c)
avaliaCmd (mem,cod) (Fun2 id x y c)    = (mem, escreve cod id cmd') where
                                         cmd' = Seq (Seq (Atr x (Var "arg")) (Atr y (Var "arg2")))
													(c)
